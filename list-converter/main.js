	 ...
	 rtb.initialize({
		extensionPoints: {
            ...
            return {
				...
				onClick: () => {
						let objects = rtb.board.selection.get()
						if (objects.length > 1) {
							var texts = []
							...
							let newList = rtb.board.widgets.create({type: 'text', text: textList, x: objects[0]['x'], y:objects[0]['y']+100, style: {fontSize: objects[0].style.fontSize}, height: height, width: width})
							//rtb.board.widgets.update(newList.id, {height: height})	
						}
						else {
							
							
								let widget = rtb.board.selection.get()
								xCoord = widget[0].x
								yCoord = widget[0].y
